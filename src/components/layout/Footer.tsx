...
const Footer: React.FC<FooterProps> = ({ onContactClick }) => {
  // --- Optimized Marquee Helper ---
  // Repeats text 6 times to ensure it covers 4K/Ultrawide screens without gaps
    ...
    <div className="flex overflow-hidden w-full" aria-hidden="true">
      <div className={`flex w-max ${direction === "left" ? "animate-marquee-left" : "animate-marquee-right"}`}>
        {[...Array(6)].map((_, i) => (
          <span 
            key={`${direction}-${i}`} 
            className="mx-8 text-3xl sm:text-4xl md:text-5xl font-semibold whitespace-nowrap text-zinc-600"
          >
            {text}
          </span>
        ))}
  ...
  return (
    <footer className="w-full bg-zinc-800 text-white pt-12 sm:pt-16 relative z-10 overflow-hidden">
      ...
      {/* --- Marquee Section (Optimized for Screen Readers) --- */}
      <div className="w-full mb-12 sm:mb-16 flex flex-col gap-4">
        {/* Screen reader only text to prevent repetitive reading */}
        <p className="sr-only">
      ...
      >
        ...
        <button
          ...
          className="flex items-center gap-3 px-6 py-3 bg-white text-zinc-800 rounded-full font-semibold text-md hover:bg-zinc-200 transition-all hover:shadow-xl active:scale-95 mb-8"
        >
          <Mail size={18} />
          {content.footer.cta}
        </button>
      ...
      {/* Spacing for mobile nav padding */}
